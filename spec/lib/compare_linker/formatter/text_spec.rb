  ...
  describe '#format' do
    ...
    context 'given downgrade' do
      context 'given owner && old_rev && new_rev' do
        ...
        it { is_expected.to eq 'compare_linker: https://github.com/masutaka/compare_linker/compare/1.1.2...1.1.1 (downgrade)' }
      end

      context 'given homepage_uri && old_tag && new_tag' do
        let(:gem_info) { build(:g_downgrade__homepage_uri__old_tag__new_tag) }
        it { is_expected.to eq 'serverspec (http://serverspec.org/): https://github.com/mizzy/serverspec/compare/v2.24.1...v2.24.0 (downgrade)' }
      end
      ...
      context 'given old_tag && new_tag' do
        ...
        it { is_expected.to eq 'chef: https://github.com/chef/chef/compare/12.4.3...12.4.1 (downgrade)' }
      end
